...
export class EntityGroupDialogComponent implements OnInit, OnDestroy {
  ...
  private entityGroupSubscription: Subscription;
  ...
  ngOnInit(): void {
    this.entityGroupSubscription = this.messageBusService
      .select<string, IEntityGroup>(EntityGroupService.MESSAGE_ENTITY_GROUP_SELECTED)
      .subscribe(group => {
  ...
  ngOnDestroy(): void {
    this.entityGroupSubscription.unsubscribe();
  }

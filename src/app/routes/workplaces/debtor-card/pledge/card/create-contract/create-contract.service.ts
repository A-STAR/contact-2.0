...
@Injectable()
export class PledgeCardCreateContractService {
  ...
  createContract(debtId: number, contract: any, pledgor: any, property: any, propertyValue: any): Observable<any> {
      ...
      .pipe(
        mergeMap(pledgorId => {
          return this.saveProperty(pledgorId, property)
            .pipe(
              mergeMap(propertyId => {
                return this.pledgeService.create(debtId, pledgorId, propertyId, contract, propertyValue);
              }),
            );
        }),
      );
